...
@Service
public class TdService extends Thread{
    ...
    private volatile static TDlibDto.newMessageRes messageRes = new TDlibDto.newMessageRes();
    private volatile static Map<String,Object> newMessage = new HashMap<>();
    private final static String TEMPLATE_GCS = "gcs";
    private final static String TEMPLATE_GC = "gc";
    private final static String TEMPLATE_SM = "sm";
...
//                    System.err.println("Unsupported command: " + command);
    ...
    private static void getMainChatList(final int limit) {
        synchronized (mainChatList) {
            ...
            StringBuffer mainChatStr = new StringBuffer("");
            for (int i = 0; i < limit && i < mainChatList.size(); i++) {
                long chatId = iter.next().chatId;
                TdApi.Chat chat = chats.get(chatId);
                synchronized (chat) {
                	mainChatStr.append(chatId);
                	mainChatStr.append(":");
                	mainChatStr.append(chat.title);
                	mainChatStr.append("\n");
                    System.out.println(chatId + ": " + chat.title);
                }
            }
            
            infoTemplate(mainChatStr.toString(),TEMPLATE_GCS);
            CMD = "";
            print("");
        }
    }
    ...
    private static void sendMessage(long chatId, String message) {
        ...
        infoTemplate(message,TEMPLATE_SM);
        CMD = "";
    }
    ...
    //設定tdlib指令
    public void setTDCommand(String command){
        CMD = command;
    }
...
//    }
    ...
    //取得訊息
    public TDlibDto.newMessageRes getMessage() {
//    	System.out.println(messageRes);
    	return messageRes;
    }
    
    //訊息模板
    private static void infoTemplate(String infos,String temp) {
    	
		messageRes.message=null;
		messageRes.title=null;
		messageRes.currency=null;
		messageRes.price=null;
		messageRes.currentProfit=null;
		messageRes.currentStopLoss=null;
		messageRes.hedging=null;
		messageRes.hedgingTime=null;
    	
    	switch(temp) {
    	case TEMPLATE_GCS:
    		messageRes.message = infos;
    		break;
    	case TEMPLATE_GC:
//        		System.out.println("object "+infos);
    		newMessage = new HashMap<>();
        	int firstIndex = infos.indexOf("text = \"");
        	int lastIndex = infos.indexOf("entities");
        	
        	if(firstIndex >= 0 && lastIndex >=0) {
        		String[] strArr = infos.substring(firstIndex + 8, lastIndex).trim().split("\n");
        		for(int i=0;i<strArr.length;i++) {
        			if(i != 0) {
        				String[] result= strArr[i].trim().split(":",2);
        				if(result[0].equals("目前獲利") || result[0].equals("套保%")) {
        					String[] result2 = result[1].split("%",2);
        					newMessage.put(result[0], result2[0]);
        				}
        				else {
        					newMessage.put(result[0], result[1]);
        				}
        			}
        			else {
        				newMessage.put("標題", strArr[i].trim());
        			}  				
        		}
        	}
		
        	for(Map.Entry<String, Object> entry : newMessage.entrySet()) {
//            		System.out.println(entry.getKey()+"--"+entry.getValue());          		
        		switch(entry.getKey()) {
        		case "標題":
        			messageRes.title = String.valueOf(entry.getValue());
        			break;
        		case "標的":
        			messageRes.currency = String.valueOf(entry.getValue());
        			break;
        		case "開倉價格":
        			messageRes.price = Double.parseDouble(entry.getValue().toString());
        			break;
        		case "目前獲利":
        			messageRes.currentProfit = Double.parseDouble(entry.getValue().toString());
        			break;
        		case "目前止損":
        			messageRes.currentStopLoss = Double.parseDouble(entry.getValue().toString());
        			break;
        		case "套保%":
        			messageRes.hedging = Double.parseDouble(entry.getValue().toString());
        			break;
        		case "套保時間":
        			String[] time = entry.getValue().toString().split("\"");
        			SimpleDateFormat formatter1=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); 
        			try {
						messageRes.hedgingTime = formatter1.parse(time[0]);
	   ...
	   private static class OrderedChat implements Comparable<OrderedChat> {
	    ...
	    private static class DefaultHandler implements Client.ResultHandler {
	        ...
	        public void onResult(TdApi.Object object) {
	            ...
	            //除了接收訊息（gc）的指令持續執行 其他指令一次只執行一次
	            if(CMD.length() > 0) {
	            	String[] commands = CMD.split(" ", 2);
	            	if(!commands[0].equals("gc")){
	            		CMD = "";
	            	}
	            	infoTemplate(object.toString(),commands[0]);
	            }
	            else {
	            	CMD = "";
	            }
